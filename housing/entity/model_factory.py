...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    """
    Description:
    This function compare multiple regression models and  return best model
    ...
    try:
        ...
        for model in model_list:
            model_name = str(model)  #getting model name based on model object #eg-> LinearRegression form LinearRegression()
            ...
            # Calculating harmonic mean of train_accuracy and test_accuracy ==>Harmonic Mean= 2(a * b)/(a + b)
            ...
            #if model accuracy is greater than base accuracy(i.e, previous best model accuracy) and train and test score is within certain thershold
...
def get_sample_model_config_yaml_file(export_dir: str):
    try:
        model_config = {
            ...
            MODEL_SELECTION_KEY: {
                "module_0": {
                    MODULE_KEY: "module_of_model", #eg:sklearn.linear_model
                    CLASS_KEY: "ModelClassName",  #eg:LinearRegression
                    PARAM_KEY:
                        {"param_name1": "value1",
                         "param_name2": "value2",
                         },
...
class ModelFactory:
    def __init__(self, model_config_path: str = None,):
        try:
            ...
            self.grid_search_cv_module: str = self.config[GRID_SEARCH_KEY][MODULE_KEY]  #-->MODULE_KEY = 'module'==grid_search key from model.yaml--
            self.grid_search_class_name: str = self.config[GRID_SEARCH_KEY][CLASS_KEY]  #returns CLASS_KEY = 'class'==GridSearchCV
            self.grid_search_property_data: dict = dict(self.config[GRID_SEARCH_KEY][PARAM_KEY]) #->PARAM_KEY = 'params' ==cv:5,verbose:2

            self.models_initialization_config: dict = dict(self.config[MODEL_SELECTION_KEY]) #-> MODEL_SELECTION_KEY = 'model_selection'
    ...
    @staticmethod
    # dynamic insertion of values for keys for model properties
    ...
    @staticmethod
    # reading parameters of model.yaml
    ...
    @staticmethod
    #Returns class_reference eg-> LinearRegression
    def class_for_name(module_name:str, class_name:str):
        try:
            # load the module, will raise ImportError if module cannot be loaded
            module = importlib.import_module(module_name)
            # get the class, will raise AttributeError if class cannot be found
            logging.info(f"Executing command: from {module} import {class_name}") #->eg:from sklearn.linear_model import LinearRegression
    ...
    # Performs grid_search = GridSearchCV(estimator, param_grid)
    ...
    #Returns List of Initialized models present in model.yaml
    def get_initialized_model_list(self) -> List[InitializedModelDetail]:
        ...
        try:
            ...
            for model_serial_number in self.models_initialization_config.keys(): #returns model_0,model_1

                model_initialization_config = self.models_initialization_config[model_serial_number]
                model_obj_ref = ModelFactory.class_for_name(module_name=model_initialization_config[MODULE_KEY],
                                                            class_name=model_initialization_config[CLASS_KEY]
                                                            )   
                model = model_obj_ref() #eg-LinearRegression
                
                #PARAM_KEY = 'params'
                if PARAM_KEY in model_initialization_config:  
                    model_obj_property_data = dict(model_initialization_config[PARAM_KEY]) #eg->min_smples_leaf:2
     ...
     # Returns best_grid_searched_model list(multiple models)
    ...
    #Compares previous best trained model with newly obtained model for best
    @staticmethod 
    ...
    #Returns Best model among the initialized models performing GridSearch
